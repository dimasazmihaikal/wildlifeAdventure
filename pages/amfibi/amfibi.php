...
<body class="bg-no-repeat bg-cover bg-[#ece2d0]">
      ...
      <div class="z-10 modal h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded shadow-lg w-1/3">
          <div class="border-b px-4 py-2">
            <h1 class="font-semibold text-3xl">AMPHIBIA</h1>
          </div>
          <div class="p-3">
          Amphibia adalah kelompok hewan yang dapat hidup di dua tempat, yaitu di air dan juga di darat. Istilah 'amfibi' berasal dari bahasa Yunani, yakni “amphi” yang berarti rangkap dan “bios” yang berarti kehidupan. Sehingga, secara harfiah ini berarti kehidupan rangkap atau dalam hal ini hidup di dua alam (air dan darat).
          </div>
  ...
  <script>
    ...
    async function fetchHewan() {
      try {
        const response = await fetch('ambil_amfibi.php');
    ...
    function displayHewan(hewan) {
      ...
      hewan.forEach(h => {
        ...
        cardImage.style.backgroundImage = `url(./../../img/${h.gambar_hewan})`;
