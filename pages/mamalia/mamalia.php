...
<body class="bg-no-repeat bg-cover bg-[#ece2d0]">
      ...
      <div class="z-10 modal h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded shadow-lg w-1/3">
          <div class="border-b px-4 py-2">
            <h1 class="font-semibold text-3xl">MAMALIA</h1>
          </div>
          <div class="p-3">
            Hewan mamalia adalah hewan yang menyusui dan merupakan hewan dengan spesies yang paling beragam di bumi. Mamalia hadir dalam bentuk dan ukuran yang berbeda-beda. Cara hidup di lingkungan dan habitatnya pun beragam. Semua mamalia berinteraksi dengan satu sama lain.
          </div>
  ...
  <script>
    ...
    async function fetchHewan() {
      try {
        const response = await fetch('ambil_mamalia.php');
