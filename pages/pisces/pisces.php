...
<body class="bg-no-repeat bg-cover bg-[#ece2d0]">
      ...
      <div class="z-10 modal h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded shadow-lg w-1/3">
          <div class="border-b px-4 py-2">
            <h1 class="font-semibold text-3xl">PISCES</h1>
          </div>
          <div class="p-3">
          Kelompok hewan ikan atau Pisces adalah kelompok vertebrata yang hidup di perairan dengan menggunakan sirip untuk bergerak dan memiliki jumlah spesies yang beraneka ragam.
          </div>
  ...
  <script>
    ...
    async function fetchHewan() {
      try {
        const response = await fetch('ambil_pisces.php');
